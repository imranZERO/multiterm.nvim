...
local default_opts = {
	height = 0.8,
	width = 0.8,
	height_value = function(self) return math.floor(vim.o.lines * self.height) end,
	width_value = function(self) return math.floor(vim.o.columns * self.width) end,
	row = function(self)
...
local function create_backdrop(tag)
	if not opts.show_backdrop then return nil, nil end
...
function M.list_terminals()
	...
	-- Set window options
	vim.api.nvim_win_set_option(win, 'winhighlight', 'NormalFloat:' .. opts.term_hl .. ',FloatBorder:' .. opts.border_hl)
	...
	vim.api.nvim_buf_set_keymap(buf, 'n', '<Esc>', '', { callback = function() vim.api.nvim_win_close(win, true) end, })
	vim.api.nvim_buf_set_keymap(buf, 'n', 'q', '', { callback = function() vim.api.nvim_win_close(win, true) end, })
